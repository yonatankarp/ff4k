 ...
 * Preserves absolute paths (those starting with '/').
 *
...
private fun joinMultipleSegments(paths: List<String>): String {
    ...
    val prefix = getPathPrefix(first)
    val normalizedFirst = removePrefix(first, prefix).trim('/')
...
private fun getPathPrefix(path: String): String = if (path.startsWith("/")) "/" else ""

private fun removePrefix(path: String, prefix: String): String = when {
    prefix.isEmpty() -> path
    prefix == "/" -> path.removePrefix("/")
    else -> path.drop(prefix.length)
}

private fun isUnixRoot(segment: String): Boolean = segment == "/"
