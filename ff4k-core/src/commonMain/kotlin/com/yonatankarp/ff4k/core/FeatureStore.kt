 ...
 * transformation, and saves the result back to the store.
 *
 * Example:
 * ```kotlin
 ...
 * @throws FeatureNotFoundException if feature doesn't exist
 */
suspend fun FeatureStore.updateFeature(
    featureId: String,
    transform: (Feature) -> Feature,
) {
    val feature = this[featureId] ?: throw FeatureNotFoundException(featureId)
    update(transform(feature))
}
 ...
 */
suspend fun FeatureStore.createOrUpdate(feature: Feature) {
 ...
 * @throws FeatureNotFoundException if feature doesn't exist
 */
suspend fun FeatureStore.toggle(featureId: String) {
    val feature = this[featureId] ?: throw FeatureNotFoundException(featureId)
 ...
 * @throws FeatureNotFoundException if feature doesn't exist
 */
suspend fun FeatureStore.getOrThrow(featureId: String): Feature = this[featureId] ?: throw FeatureNotFoundException(featureId)
 ...
 */
suspend fun FeatureStore.isEmpty(): Boolean = getAll().isEmpty()
 ...
 */
suspend fun FeatureStore.count(): Int = getAll().size
