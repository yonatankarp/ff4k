...
class JdbcFeatureStoreTest : FunSpec({
    ...
    context("update with transform") {
        ...
        test("throws IllegalStateException after max retries due to concurrent modifications") {
            ...
            val connection = mockk<Connection> {
                every { prepareStatement(PostgresDialect.selectFeatureByUidSql) } returns selectStatement
                every { prepareStatement(PostgresDialect.updateFeatureSql) } returns updateStatement
